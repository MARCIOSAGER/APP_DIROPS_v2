...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`📅 Iniciando envio de relatórios mensais para ${aeroportosParaProcessar.length} aeroporto(s)`);
    ...
    return Response.json({
      ...
      erros,
      resultados,
      mensagem: `Relatório mensal gerado para ${aeroportosParaProcessar.length} aeroporto(s)`
    });

  } catch (error) {
    console.error('❌ Erro crítico:', error);
    return Response.json({ 
      error: 'Erro ao enviar relatórios mensais',
