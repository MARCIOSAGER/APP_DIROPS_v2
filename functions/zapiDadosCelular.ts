...
Deno.serve(async (req) => {
    try {
        ...
        const zapiUrl = `https://api.z-api.io/instances/${instanceId}/token/${encodedToken}/me`;
        ...
        if (!response.ok) {
            ...
            return Response.json({ 
                success: false,
                error: `Erro ao obter dados do celular: ${response.status}`,
                details: errorText
        ...
        console.error('Erro ao obter dados do celular Z-API:', error);
        return Response.json({
            success: false,
            error: error.message
        }, { status: 500 });
