...
Deno.serve(async (req) => {
    try {
        ...
        if (user.role !== 'admin' && !(user.perfis && user.perfis.includes('administrador'))) {
            return Response.json({ error: 'Acesso negado. Apenas administradores podem desconectar a instância.' }, { status: 403 });
        }
        ...
        const zapiUrl = `https://api.z-api.io/instances/${instanceId}/token/${encodedToken}/disconnect`;
        ...
        if (!response.ok) {
            ...
            return Response.json({ 
                success: false,
                error: `Erro ao desconectar instância: ${response.status}`,
                details: errorText
        ...
        console.log('Instância Z-API desconectada com sucesso:', data);
