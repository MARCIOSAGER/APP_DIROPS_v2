-- 051: Update fetch_fr24 to support historical data via /history/ prefix
-- FlightAware AeroAPI Standard plan: /history/airports/{id}/flights/arrivals for data >9 days old
...
DECLARE
  ...
  dest jsonb;
BEGIN
  -- Use /history/ prefix for dates older than 9 days
  IF p_date_from::timestamp < (now() - interval '9 days') THEN
    prefix := '/history';
  END IF;
  ...
  arr_resp := http((
    'GET',
    'https://aeroapi.flightaware.com/aeroapi' || prefix || '/airports/' || p_airport || '/flights/arrivals?max_pages=2&start=' || p_date_from || '&end=' || p_date_to,
  ...
  dep_resp := http((
    'GET',
    'https://aeroapi.flightaware.com/aeroapi' || prefix || '/airports/' || p_airport || '/flights/departures?max_pages=2&start=' || p_date_from || '&end=' || p_date_to,
    ARRAY[http_header('x-apikey', api_key)],
    NULL, NULL
  )::http_request);
