...
export default function SearchProgressBar({ progress, totalFlights, elapsedTime, estimatedTime, currentPage, error, isComplete }) {
  ...
  const getStatusMessage = () => {
    if (error) return 'Erro na Pesquisa';
    if (isComplete) return 'Pesquisa Concluída com Sucesso';
    return `Extraindo Voos (Página ${currentPage})...`;
  };
      ...
      <CardContent className="p-4 space-y-3">
        ...
        {!error && (
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div className="bg-white rounded p-2 border border-slate-200">
              <div className="text-slate-500 text-xs">Tempo Decorrido</div>
            ...
            <div className="bg-white rounded p-2 border border-slate-200">
              ...
              <div className="font-semibold text-slate-800">{isComplete ? 'Completo' : formatTime(estimatedTime)}</div>
            </div>
            <div className="bg-white rounded p-2 border border-slate-200">
              <div className="text-slate-500 text-xs">Tempo Restante</div>
              <div className="font-semibold text-slate-800">
                {isComplete ? 'Finalizado' : (estimatedTime > elapsedTime ? formatTime(estimatedTime - elapsedTime) : 'Finalizando...')}
              </div>
            </div>
          </div>
        )}
        ...
        {error && (
          <div className="p-3 bg-red-100 border border-red-300 rounded">
            <div className="text-xs font-semibold text-red-800 mb-1">❌ Erro ao Buscar Voos</div>
        ...
        {isComplete && !error && (
          <div className="p-3 bg-green-100 border border-green-300 rounded">
            <div className="text-xs font-semibold text-green-800">✅ {progress} voo(s) pronto(s) para importação</div>
            <div className="text-xs text-green-700 mt-1">A tabela abaixo foi atualizada com os resultados.</div>
          </div>
        )}
        ...
        {!error && !isComplete && (
          <div className="p-2 bg-yellow-100 border border-yellow-300 rounded text-xs text-yellow-700">
            ⏱️ Limite de 10 requisições por minuto. Aguardando conforme necessário...
          </div>
        )}
      </CardContent>
    </Card>
