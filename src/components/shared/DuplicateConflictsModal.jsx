    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-xl">{t('shared.conflitos.titulo')}</DialogTitle>
          <DialogDescription className="text-center text-slate-500 dark:text-slate-400 pt-2">
            {t('shared.conflitos.descricao')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="my-6 space-y-4">
          <div className="flex justify-between items-center bg-green-50 dark:bg-green-950 p-4 rounded-lg border border-green-200 dark:border-green-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-green-800 dark:text-green-200">{t('shared.conflitos.novos_registos')}</p>
                <p className="text-sm text-green-700 dark:text-green-300">{t('shared.conflitos.serao_adicionados')}</p>
              </div>
            </div>
          ...
          <div className="flex justify-between items-center bg-yellow-50 dark:bg-yellow-950 p-4 rounded-lg border border-yellow-200 dark:border-yellow-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-yellow-800 dark:text-yellow-200">{t('shared.conflitos.registos_duplicados')}</p>
                 <p className="text-sm text-yellow-700 dark:text-yellow-300">{t('shared.conflitos.pode_ignorar')}</p>
              </div>
            </div>
        ...
        <DialogFooter className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Button
            ...
            title={newRecordsCount === 0 ? t('shared.conflitos.nenhum_novo') : ""}
          >
            <PlusCircle className="mr-2 h-4 w-4" />
            {t('shared.conflitos.ignorar_duplicados')} ({newRecordsCount} {t('shared.conflitos.novos')})
          </Button>
          <Button
          ...
          >
            ...
            {t('shared.conflitos.atualizar_existentes')} ({conflictsCount + newRecordsCount} Total)
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
